...
void print_info(struct pstat *pstat, int j) {
...
int find_pid(struct pstat *pstat, int pid) {
    int i;
    for (i = 0; i < NPROC; i++) {
        if (pstat->pid[i] == pid) {
            return i;
...
int main(int argc, char *argv[]) {
    ...
    // int n_time = atoi(argv[1]); /* You can pass the number of time-steps as a
    // command line argument if you uncomment this. Hard-coded for now. */
